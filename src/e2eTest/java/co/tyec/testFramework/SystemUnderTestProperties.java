...
public class SystemUnderTestProperties
{
    ...
    private static void loadSutPropfile()
    {
        ...
        {
            Assert.fail("sut.propfile has not been set. Initialization failed. Please set sut.propfile via java command line -Dsut.propfile=filename or add a default to SutDefaults.properties");
        }
    ...
    public static void loadPropFileStreamIntoProperties(InputStream propFileInputStream, String propFilename)
    {
        ...
        try
        {
            ...
            {
                String fromFile = propertiesHandler.getProperty(name);
                String fromSys = System.getProperty(name, sutProperties.getProperty(name));
                if (fromSys == null || fromSys.isEmpty())
                {
                    sutProperties.setProperty(name, fromFile);
                    System.out.println(String.format("%s - Set: [%s = %s]", propFilename, name, fromFile));
                }
                else
                {
                    sutProperties.setProperty(name, fromSys);
                    System.out.println(String.format("%s - Ignored: [%s = %s] System value: [%s]", propFilename, name, fromFile, fromSys));
                }
            }
        }
        catch (IOException e)
